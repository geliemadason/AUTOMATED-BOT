...
module.exports.onLoad = async function() {
  ...
  const path = __dirname + "/system/chat.json";
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (isAdmin) {
    if (args[0] === "off") {
      ...
      return api.sendMessage(`chat off has been activated. the bot will now remove non-admin members from the group when they chat.`, event.threadID);
    } else if (args[0] === "on") {
