...
module.exports.run = async function({ api, event, args }) {
  ...
  api.getUserInfo(targetID, async (err, result) => {
    if (err) {
      return api.sendMessage(formatFont(`Failed to retrieve user info: ${err.message}`), tid, mid);
    }

    const userName = result[targetID]?.name || "User";
    const outputPath = __dirname + `/cache/rip_${tid}_${mid}.png`;

    try {
      const apiUrl = `https://api-canvass.vercel.app/rip?userid=${targetID}`;
      ...
      writer.on("finish", () => {
        api.sendMessage({
          body: formatFont(`RIP ${userName} ⚰️`),
